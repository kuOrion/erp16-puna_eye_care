...
patch(OptionalProductsModal.prototype, 'sh_product_configurator_always_clickable_prototype', {
    _toggleDisable: function ($parent, isCombinationPossible) {
        $parent.toggleClass('css_not_available', !isCombinationPossible);
        if ($parent.hasClass('in_cart')) {
            const primaryButton = this.$footer.find('.btn-primary');
...
patch(SaleOrderLineProductField.prototype, 'sh_product_configurator', {

    async _openProductConfigurator(mode) {
        ;
        if (this.env.model.root.resModel == 'product.product'){
                ;
                localStorage.setItem('sh_active_model', this.env.model.root.resModel);
                ...
                ;
                var model_context = this.context
                model_context['active_model'] = this.env.model.root.resModel
                const $modal = $(
                    await this.rpc(
                ...
                localStorage.setItem('sh_productId', productId);
                ...
                optionalProductsModal.opened(() => {
                    ...
                    optionalProductsModal._onModalReady(); // Call the new method
                });
                ...
                optionalProductsModal.on("confirm", null, async () => {
                    confirmed = true;
                    ;
                    const [mainProduct, ...optionalProducts] = await optionalProductsModal.shGetAndCreateSelectedProducts(this.props.record.data.id);

                    var is_product_unlink = await this.rpc(
                                "/unlink/product_variant",
                                {
                                    product_data : productId
                                },
                            )

                    if(is_product_unlink){
                        localStorage.removeItem('sh_productId');
                    }


                    ;
        //            var product_data = await this._convertConfiguratorDataToUpdateData(mainProduct);
